                    ...
                    {projects.map((project, idx) => (
                        <div key={idx} className="project-card group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-sm transition-all duration-300 border border-gray-100 flex flex-col h-full">
                            <div className="relative h-64 w-full overflow-hidden">
                                ...
                                />
                                <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider text-green-600 shadow-sm">
                                    {project.status}
                                </div>
                            </div>
                            <div className="p-8 flex-1 flex flex-col">
                                <span className="text-green-500 text-sm font-semibold mb-3">{project.category}</span>
                                <h3 className="text-2xl font-bold heading-text text-gray-900 mb-4 group-hover:text-green-600 transition-colors">{project.title}</h3>
